# Runtime Tcl commands to interact with - vexriscv_with_fpu_v1_0

# Sourcing design address info tcl
set bd_path [get_property DIRECTORY [current_project]]/[current_project].srcs/[current_fileset]/bd
source ${bd_path}/vexriscv_with_fpu_v1_0_include.tcl
...
# Test: IMEM_AXI
set wdata_imem_axi [get_init_data 0]
create_hw_axi_txn w_imem_axi_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_imem_axi}
create_hw_axi_txn r_imem_axi_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_imem_axi_addr
run_hw_axi w_imem_axi_addr
run_hw_axi r_imem_axi_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_imem_axi_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 2 0 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - IMEM_AXI"
} else {
	puts "Data comparison test fail for - IMEM_AXI, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}

# Test: DMEM_AXI
set wdata_dmem_axi [get_init_data 1]
create_hw_axi_txn w_dmem_axi_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_dmem_axi}
create_hw_axi_txn r_dmem_axi_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_dmem_axi_addr
run_hw_axi w_dmem_axi_addr
run_hw_axi r_dmem_axi_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_dmem_axi_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 2 1 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - DMEM_AXI"
} else {
	puts "Data comparison test fail for - DMEM_AXI, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}
