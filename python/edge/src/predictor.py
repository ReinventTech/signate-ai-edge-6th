...
class ScoringService(object):
    ...
    def get_model(cls, model_path):
        ...
        """
        # xir_graph = xir.Graph.deserialize(os.path.join(model_path, "bev.xmodel"))
        # xir_root_subgraph = xir_graph.get_root_subgraph()
        # xir_child_subgraphs = xir_root_subgraph.toposort_child_subgraph()
        # cls.runner = vart.Runner.create_runner(xir_child_subgraphs[1], "run")
        cls.runner = None
        cls.count = 0
    ...
    def predict(cls, input):
        ...
        bev_pedestrian_preds, bev_vehicle_preds = bev_predictor.predict(
            cls.runner, lidar.copy(), cam_ego_pose, cam_calib, lidar_calib
        )
